...
export default function Navbar() {
  const linkClass = ({ isActive }: { isActive: boolean }) =>
    `flex items-center gap-2.5 py-2 px-3 rounded-lg text-sm transition-colors ${
      isActive
        ? "bg-claude-sidebar-active text-white font-medium"
        : "text-gray-400 hover:text-white hover:bg-claude-sidebar-hover"
    }`;

  return (
    <aside className="w-52 bg-claude-sidebar min-h-screen flex flex-col">
      <Link to="/" className="px-5 py-5">
        <div className="flex items-center gap-2.5">
          ...
          <span className="font-semibold text-white">AutoIntegrate</span>
        </div>
      </Link>
      ...
      <div className="border-t border-claude-sidebar-hover p-3">
        <UserMenu />
      </div>
    </aside>
  );
